...
--- 提取参数说明
--- end_grid_id, od矩阵中起点网格的id，作为计算的唯一标识
...
select
    ...
    count(distinct start_grid_id) as place_num,
    a.end_grid_id
from
    flow_temp a
group by
...
from
    flow_temp a
    left join grid_stay_sum_cache b on a.end_grid_id = b.end_grid_id;
...
from
    ...
    left join grid_stay_sum_cache b on c.end_grid_id = b.end_grid_id;
