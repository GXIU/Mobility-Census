def get_flow_in_out_ratio(trj_df,origin_label,destination_label):
    ...
    #计算出入流
    out_df = grouped_df[[origin_label,'weight']].groupby(origin_label).sum().reset_index()
    in_df = grouped_df[[destination_label,'weight']].groupby(destination_label).sum().reset_index()
    ratio_df = out_df.merge(right = in_df,left_on = origin_label,right_on = destination_label,suffixes=['_out','_in']).fillna(0)
